...
- Generated by running it: `Assets/Scenes/TestScene.unity`, a `Ground` layer in `ProjectSettings/TagManager.asset`
- Uses (read-only): Unity's built-in square sprite at `Packages/com.unity.2d.sprite/Editor/ObjectMenuCreation/DefaultAssets/Textures/v2/Square.png` — 256×256 at 256 PPU = exactly 1×1 world unit, same asset the GameObject > 2D Object > Sprites > Square menu uses. No sprite is generated.
...
```bash
cd "/Users/vanshsrivastava/Unity/GMTK(Input-Lag)"
git add Assets/Assets/Scripts Assets/Scenes/TestScene.unity ProjectSettings/TagManager.asset
